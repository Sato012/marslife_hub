...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///marslife.db'  # Will be replaced with PostgreSQL
...
class User(db.Model):
    ...
    role = db.Column(db.String(20), default='astronaut')
...
def get_resources():
    ...
    default_resources = [
        ...
        {'name': 'Пища', 'current': 1200, 'max': 2000, 'unit': 'кг', 'icon': 'coffee'},
    ]
...
def transactions():
    ...
    return render_template('transactions.html', transactions=user_transactions, products=products)
...
def init_db():
    with app.app_context():
        ...
        if User.query.count() == 0:
            ...
            # # Create products
            # products = [
            #     Product(name='Кислородные баллоны', description='Дополнительный запас кислорода для аварийных ситуаций',
            #             price=1200.0, stock=5, category='Жизнеобеспечение', image='placeholder.jpg'),
            #     Product(name='Фильтр для воды', description='Высокоэффективный фильтр для очистки воды',
            #             price=350.0, stock=20, category='Системы очистки воды', image='placeholder.jpg'),
            #     Product(name='Ремонтный набор', description='Набор инструментов для ремонта оборудования',
            #             price=180.0, stock=15, category='Agriculture', image='placeholder.jpg'),
            #     Product(name='Аварийный рацион', description='Запас пищи на 7 дней для аварийных ситуаций',
            #             price=90.0, stock=8, category='Питание', image='placeholder.jpg'),
            #     Product(name='Медицинский набор', description='Базовый набор для оказания первой помощи',
            #             price=2500.0, stock=3, category='Медицина', image='placeholder.jpg')
            # ]
            # db.session.add_all(products)
            #
            # db.session.commit()

        # Обновляем названия существующих товаров на русский язык
        update_product_names_on_startup()


def update_product_names_on_startup():
    try:
        # Словарь для перевода названий продуктов
        product_translations = {
            'Oxygen Tank': 'Кислородный баллон',
            'Water Filter': 'Фильтр для воды',
            'Solar Panel Cleaning Kit': 'Солнечная панель',
            'Emergency Ration': 'Аварийный рацион',
            'Repair Kit': 'Ремонтный набор',
            'Medical Kit': 'Медицинский набор',
            'Oxygen Filter': 'Кислородный фильтр',
            'Water Purification Tablets': 'Таблетки для очистки воды',
            'Nutrient Solution A': 'Питательный раствор А',
            'Кислородные баллоны': 'Кислородный баллон'
        }

        # Словарь для перевода категорий
        category_translations = {
            'Life Support': 'Жизнеобеспечение',
            'Water Systems': 'Очистка',
            'Food': 'Питание',
            'Maintenance': 'Инструменты',
            'Medical': 'Медицина',
            'Agriculture': 'Удобрение',
        }

        # Словарь для перевода описаний
        description_translations = {
            'High-efficiency oxygen filter for life support systems': 'Высокоэффективный кислородный фильтр для систем жизнеобеспечения',
            'Advanced water purification tablets': 'Высокоэффективные таблетки для очистки воды',
            'Specialized cleaning solution for Mars dust': 'Дополнительная солнечная панель для генерации электроэнергии',
            'Essential nutrients for hydroponics': 'Основные питательные вещества для гидропоники'
        }

        # Получаем все продукты
        products = Product.query.all()

        # Обновляем названия, категории и описания
        for product in products:
            # Обновляем название, если есть перевод
            if product.name in product_translations:
                product.name = product_translations[product.name]

            # Обновляем категорию, если есть перевод
            if product.category in category_translations:
                product.category = category_translations[product.category]

            # Обновляем описание, если есть перевод
            if product.description in description_translations:
                product.description = description_translations[product.description]

        # Сохраняем изменения
        db.session.commit()
        print("Названия товаров успешно обновлены на русский язык")
    except Exception as e:
        db.session.rollback()
        print(f"Ошибка при обновлении названий товаров: {str(e)}")
